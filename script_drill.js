...
function activatePipeLengthField(group) {
  ...
  if (group === "Группа длин 1 (от 6,1 м до 7,01 м)") {
    defaultValue = "6.4";
    min = 6.1;
    max = 7.01;
  } else if (group === "Группа длин 2 (от 8,84 м до 9,75 м)") {
    defaultValue = "8.84";
    min = 8.84;
    max = 9.75;
  } else if (group === "Группа длин 3 (от 12,19 м до 13,72 м)") {
    defaultValue = "12.19";
    min = 12.19;
    max = 13.72;
  }
...
function activateCustomLengthField(id, values) {
  ...
  hint.textContent = `Введите значение от ${defaultVal} мм до ${max} мм`;
  hint.style.display = "none";
